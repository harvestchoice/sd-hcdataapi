...
#pragma warning restore 0219
    ...
    public static class MyQueryExtensions
    {
        ...
        public static object Average(this IQueryable source, string member)
        {
            ...
            MethodInfo avgMethod = typeof(Queryable).GetMethods().First(m => m.Name == "Average" && m.ReturnType == prop.PropertyType && m.IsGenericMethod);

            return source.Provider.Execute(Expression.Call(null, avgMethod.MakeGenericMethod(new Type[] { source.ElementType }), new Expression[] { source.Expression, Expression.Quote(selector) }));

        }

        public static object Average<TSource>(this IQueryable<TSource> source, string member)
        {
            ...
            return source.Provider.Execute(Expression.Call(null, avgMethod.MakeGenericMethod(new Type[] { typeof(TSource) }), new Expression[] { source.Expression, Expression.Quote(selector) }));
